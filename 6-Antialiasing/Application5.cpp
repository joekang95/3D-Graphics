...
int Application5::Render() 
{
	...
	*/ 
	...
	/* HW6 */
	Antialising();
	m_pFrameBuffer = m_pRender[5]->framebuffer; // Set framebuffer
	m_pRender[5]->GzFlushDisplay2File(outfile); 	/* write out or update display to file*/
	m_pRender[5]->GzFlushDisplay2FrameBuffer();	// write out or update display to frame buffer
...
int Application5::Antialising() {
	for (int i = 0; i < m_nWidth * m_nHeight; i++) {
		...
		for (int j = 0; j < AAKERNEL_SIZE; j++) {
			final_pixel.red += m_pRender[j]->pixelbuffer[i].red * AAFilter[j][2];
			final_pixel.green += m_pRender[j]->pixelbuffer[i].green * AAFilter[j][2];
			final_pixel.blue += m_pRender[j]->pixelbuffer[i].blue * AAFilter[j][2];
			final_pixel.z += m_pRender[j]->pixelbuffer[i].z * AAFilter[j][2];

		}
		m_pRender[5]->pixelbuffer[i] = final_pixel;
	}
	return GZ_SUCCESS;
}
